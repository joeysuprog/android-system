...
>  深入学习 ActivityManagerService 与 WindowManagerService
...
1. **addWindow**：`WMS.addWindow() 的完整逻辑是什么？如何创建 WindowState 和 SurfaceControl？`
2. **层级关系**：`DisplayContent、WindowToken、WindowState 三者的继承和包含关系是怎样的？`
3. **WMS 与 SF**：`WMS 创建的 SurfaceControl 如何传递给 SurfaceFlinger 参与合成？`

### 调试与排查

1. **dumpsys 解读**：`dumpsys activity activities 输出的 RootTask、Task、Activity 结构如何解读？`
2. **窗口问题**：`应用窗口无法显示时，应该从 dumpsys window 的哪些字段入手排查？`
