...
> 将 Android渲染子系统 的核心内容合成为一条完整的渲染流水线，建立从 Input 到 Display 的全链路视角
>
> 1. [《View绘制体系》](./View绘制体系.md)
> 2. [《HWUI渲染管线》](./HWUI渲染管线.md)
> 3. [《VSync与帧调度》](./VSync与帧调度.md)
> 4. [《SurfaceFlinger合成》](./SurfaceFlinger合成.md)

---

## 目录

1. [从触摸到像素上屏的完整链路](#1-从触摸到像素上屏的完整链路)
2. [关键瓶颈点分析](#2-关键瓶颈点分析)
3. [源码路径速查表](#3-源码路径速查表)
4. [AI 交互建议](#4-ai-交互建议)
...
| **源码路径** | `frameworks/native/services/inputflinger/reader/InputReader.cpp` `frameworks/native/services/inputflinger/reader/InputReader.h` |
...
| **源码路径** | `frameworks/native/services/inputflinger/dispatcher/InputDispatcher.cpp` `frameworks/native/services/inputflinger/dispatcher/InputDispatcher.h` |
...
| **源码路径** | `frameworks/base/core/java/android/view/ViewRootImpl.java` （方法：`enqueueInputEvent`, `deliverInputEvent`, `processInputEvent`） |
...
| **源码路径** | `frameworks/base/core/java/android/view/Choreographer.java` （方法：`doFrame`, `doCallbacks`） |
...
| **源码路径** | `frameworks/base/core/java/android/view/ViewRootImpl.java` （方法：`performTraversals`, `performMeasure`, `performLayout`, `performDraw`） |
...
| **源码路径** | `frameworks/base/libs/hwui/ThreadedRenderer.cpp` `frameworks/base/libs/hwui/renderthread/CanvasContext.cpp` |
...
| **源码路径** | `frameworks/base/libs/hwui/renderthread/RenderThread.cpp` `frameworks/base/libs/hwui/renderthread/CanvasContext.cpp` `frameworks/base/libs/hwui/SkiaDisplayList.cpp` |
...
| **源码路径** | `frameworks/native/libs/gui/BufferQueue.cpp` `frameworks/native/libs/gui/BufferQueueProducer.cpp` |
...
| **源码路径** | `frameworks/native/services/surfaceflinger/SurfaceFlinger.cpp` （方法：`onMessageRefresh`, `composite`, `rebuildLayerStacks`）         |
...
| **核心逻辑** | 硬件逐行扫描像素到屏幕                                                                                                |
| **源码路径** | `frameworks/native/services/surfaceflinger/DisplayHardware/` HAL: `hardware/interfaces/graphics/composer/` |
...
- [View绘制体系](./View绘制体系.md)  
- [HWUI渲染管线](./HWUI渲染管线.md)  
- [SurfaceFlinger合成](./SurfaceFlinger合成.md)  
- [VSync与帧调度](./VSync与帧调度.md)
