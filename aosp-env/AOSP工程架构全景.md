...
1. [一、Android 系统五层架构](#一android-系统五层架构)
2. [二、AOSP 顶层目录全览](#二aosp-顶层目录全览)
3. [三、`frameworks/` 深度展开 — 最核心的目录](#三frameworks-深度展开-最核心的目录)
4. [四、`packages/` — 系统内置 App](#四packages-系统内置-app)
5. [五、`system/` — 系统底层](#五system-系统底层)
6. [六、其他重要目录](#六其他重要目录)
7. [七、进程模型](#七进程模型)
8. [八、构建系统简介](#八构建系统简介)
9. [九、学习优先级路线图](#九学习优先级路线图)
...
flowchart TB
    subgraph L1 [应用层 Applications]
        direction LR
        Launcher3["Launcher3 桌面"] ~~~ Settings["Settings 设置"] ~~~ SystemUI_app["SystemUI 状态栏/通知"] ~~~ Camera["Camera 相机"] ~~~ Dialer["Dialer 电话"] ~~~ more1["...50+ 系统 App"]
    end

    subgraph L2 [Java Framework 层]
        direction LR
        AMS["ActivityManagerService\n活动管理"] ~~~ WMS["WindowManagerService\n窗口管理"] ~~~ PMS["PackageManagerService\n包管理"] ~~~ ViewSystem["View / ViewRootImpl\n视图体系"] ~~~ Choreographer_f["Choreographer\n帧调度"] ~~~ InputManager["InputManagerService\n输入管理"] ~~~ NotificationMgr["NotificationManager\n通知管理"]
    end

    subgraph L3 [Native Framework 层]
        direction LR
        SF["SurfaceFlinger\n图形合成"] ~~~ IF["InputFlinger\n输入处理"] ~~~ AF["AudioFlinger\n音频处理"] ~~~ HWUI_n["HWUI\n硬件加速渲染"] ~~~ BQ["BufferQueue\n缓冲队列"] ~~~ BinderNative["Binder IPC\n进程间通信"]
    end

    subgraph L4 [HAL 硬件抽象层]
        direction LR
        HWC["HWComposer\n显示合成"] ~~~ Gralloc["Gralloc\n图形内存分配"] ~~~ AudioHAL["Audio HAL\n音频硬件"] ~~~ CameraHAL["Camera HAL\n相机硬件"] ~~~ SensorHAL["Sensor HAL\n传感器"]
    end

    subgraph L5 [Linux Kernel 内核层]
        direction LR
        BinderDrv["Binder 驱动"] ~~~ DisplayDrv["Display 驱动"] ~~~ InputDrv["Input 驱动"] ~~~ AudioDrv["Audio 驱动"] ~~~ GPU["GPU 驱动"]
    end
...
AOSP 根目录下共 26 个子目录。下表按功能分组，标注学习优先级。
...
| 目录                | 大小   | 内容                                         | 学习重点                      |
| ----------------- | ---- | ------------------------------------------ | ------------------------- |
| `**frameworks/`** | 2.2G | **Android 系统框架**，包含 Java API 层和 Native 服务层 | ★★★ 最核心目录，应用开发核心关注 |
