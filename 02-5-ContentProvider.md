...
> 面向具有 5.5 年 Android 应用开发经验的开发者，从 AOSP 源码视角全面解析 ContentProvider 的跨进程数据共享模型、Content URI 设计、query/insert 完整链路、ContentObserver 观察机制、权限与安全、FileProvider 原理，以及 Scoped Storage 演进
...
4. `ContentProvider.Transport 的 query 方法中做了哪些权限检查？调用链是什么？`
5. `Stable 和 Unstable Provider 连接在源码中的区别是什么？AMS 如何管理这两种连接的引用计数？`

### 数据观察

6. `ContentService 中的 ObserverNode 树是如何组织的？notifyChange() 是如何匹配和分发的？`
7. `ContentObserver 的 onChange 回调是在哪个线程执行的？Handler 参数的作用是什么？`

### 权限与安全

8. `URI 临时权限授予在源码中的实现路径是什么？UriGrantsManagerService 如何记录和检查临时权限？`
9. `FileProvider.getUriForFile() 的路径映射逻辑是怎样的？它是如何防止路径遍历攻击的？`
