# Day 11：AI 模拟性能实验室 2 —— App 冷启动优化

> 面向具有 5.5 年 Android 应用开发经验的开发者，通过真实的线上问题模拟，掌握从复现、采集、分析到优化的完整冷启动排查方法论
...
这是一个典型的 **Cold Start（冷启动）** 性能问题。作为有过 5 年+ 经验的开发者，你可能已经了解 Application.onCreate、懒加载等基础手段，但面对「各阶段具体耗时如何拆分」「Perfetto 如何精确定位瓶颈」「如何设计可落地的优化方案」时，仍需要系统化的工具和方法。本节将通过 AI 模拟的 bug 报告，带你走完一次真实的冷启动优化实战流程。
...
| 指标 | 含义 |
|-----|------|
| **Pss** (Proportional Set Size) | 按共享比例分摊后的内存 |
| **Private Dirty** | 仅本进程使用的物理内存 |
| **Java Heap** | Java 堆占用 |
| **Native Heap** | Native 堆占用 |
| **Code** | 代码段（dex、so） |
| **Stack** | 线程栈 |
| **Graphics** | GPU 相关内存 |
...
使用 **Android Studio Profiler** 或 **Eclipse MAT** 分析 heap dump，查找泄漏路径。

### 4.3 启动过程内存分析

在 Perfetto 中查看 **memory** track 的 counters —— 监控启动过程中的内存增长曲线，可辅助判断：

- 是否存在启动阶段的内存陡增（如一次性加载大量资源）
- GC 频率是否异常
- Native/Java 堆增长是否合理
