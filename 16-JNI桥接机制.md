...
> 面向具有 5.5 年 Android 应用开发经验的开发者，系统讲解 JNI（Java Native Interface）在 JVM 与 Native（C/C++）之间的桥梁作用，涵盖核心 API（JavaVM/JNIEnv）、数据类型映射、静态/动态注册、双向调用链路、引用管理，并深入 AOSP 源码展示 Android Framework 如何大规模使用 JNI 连接 Java 层与 Native 层
...
5. `帮我分析 android_os_Binder.cpp 中 BinderProxy.transact() 的 JNI 实现，数据是如何从 Java Parcel 转到 Native Parcel 的？`
6. `android_view_Surface.cpp 中 nativeLockCanvas() 的实现链路是什么？Canvas 是怎么从 Native 层传回 Java 层的？`
7. `Zygote fork 进程时，JNI 层做了哪些事情？为什么 fork 后不需要重新注册 JNI 方法？`

### 调试排查

8. `如何用 CheckJNI 模式排查 JNI 引用泄漏？有什么典型的报错信息？`
9. `tombstone 文件中的 JNI 崩溃堆栈怎么看？如何定位到是哪个 JNI 函数出了问题？`
10. `如何监控一个进程的 Global Reference 数量？数量异常增长怎么排查？`

---

## 13. 关联文档

- [01-Android系统架构全景](./01-Android系统架构全景.md) — JNI 在 Framework Java 层与 Native 层之间的分层位置
- [02-Binder与跨进程通信](./02-Binder与跨进程通信.md) — Binder 的 Java/Native 桥接依赖 JNI（`android_os_Binder.cpp`）
- [01-1-Android系统启动流程](./01-1-Android系统启动流程.md) — Zygote 启动时通过 JNI 注册 Framework 方法
- [01-2-App启动流程](./01-2-App启动流程.md) — App 进程继承 Zygote 的 JNI 注册表
- [03-2-Flutter渲染框架](./03-2-Flutter渲染框架.md) — Flutter Platform Channel 底层使用 JNI 桥接 Dart ↔ Java
- [02-2-Activity与Fragment](./02-2-Activity与Fragment.md) — Activity 生命周期的 ClientTransaction 从 Java 层穿越 JNI
