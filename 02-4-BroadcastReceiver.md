...
> 面向具有 5.5 年 Android 应用开发经验的开发者，从 AOSP 源码视角全面解析 BroadcastReceiver 的发布-订阅模型、静态/动态注册机制、sendBroadcast 完整分发链路、有序广播、权限安全、ANR 超时，以及 Android 8–15 的广播限制演进
...
4. `有序广播的串行分发在 BroadcastQueue 中是如何实现的？如何保证一个接收者处理完才分发给下一个？`
5. `abortBroadcast() 的实现原理是什么？在源码中是哪个字段控制的？`

### 权限与安全

6. `broadcastIntentLocked() 中的权限检查逻辑是怎样的？发送端权限和接收端权限分别在哪里校验？`
7. `Android 14 的 RECEIVER_EXPORTED 校验逻辑在 AMS 源码的什么位置？`

### ANR 与性能

8. `BroadcastQueue 的 ANR 超时计时器是如何设置和触发的？超时后的处理流程是什么？`
9. `Android 8 的隐式广播过滤逻辑在源码中的什么位置？白名单是如何维护的？`
